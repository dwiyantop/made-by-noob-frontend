...
export interface ImageLoaderProps extends Omit<React.ComponentProps<typeof Image>, 'onError'> {
...
export function ImageLoader({
  ...
  if (hasError && showFallbackLogo) {
    const fallbackContent = (
      <>
        ...
        <div className="relative z-10 flex items-center justify-center">
          <Image src="/favicon.svg" alt={alt || 'MadeByNoob'} width={64} height={64} className="opacity-80" />
        </div>
      </>
    );
    ...
    if (isFill) {
      return (
        <div className={cn('absolute inset-0 flex items-center justify-center bg-card/90', fallbackClassName)}>
          {fallbackContent}
        </div>
      );
    }
    ...
    return (
      <div className={cn('relative flex items-center justify-center bg-card/90', className, fallbackClassName)}>
        {fallbackContent}
      </div>
    );
  }
