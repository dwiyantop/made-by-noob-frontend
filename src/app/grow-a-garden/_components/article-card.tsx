...
interface ArticleCardProps {
  ...
  description: string;
  category: string;
  categoryColor: 'success' | 'warning' | 'info';
  readTime: string;
  imageUrl: string;
}
      ...
      <div className="relative aspect-3/2 w-full">
        ...
        <div className="pointer-events-none absolute inset-x-0 bottom-0 z-30 space-y-1.5 px-5 pb-5 pt-12">
          <div className="flex items-center justify-between">
            <Heading variant="h3" className="text-2xl font-bold text-text-primary line-clamp-2 flex-1">
              {title}
            </Heading>
            <span className="ml-3 shrink-0 text-xs text-text-primary/60">{readTime}</span>
          </div>
